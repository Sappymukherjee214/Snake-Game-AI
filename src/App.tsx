...
import { Play, Pause, RotateCcw, Brain, Activity, Target, Zap, Save, Download, Upload, ChevronRight } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.beginPath(); ctx.arc(food.x * cellSize + cellSize / 2, food.y * cellSize + cellSize / 2, cellSize / 3, 0, Math.PI * 2);
      ...
      snake.forEach((p, i) => {
        ...
        if (isHead) { ctx.shadowBlur = 10; ctx.shadowColor = '#38bdf8'; }
        const padding = isHead ? 1 : 2;
        ctx.fillRect(p.x * cellSize + padding, p.y * cellSize + padding, cellSize - padding * 2, cellSize - padding * 2);
      });
      ctx.shadowBlur = 0;
    };
      ...
      if (!isPaused) {
        if (time - lastTimeRef.current >= speed) {
          ...
          if (done) {
            ...
            game.reset(); setTotalReward(0); setEpsilon(agent.getEpsilon());
            if (episode % 10 === 0) agent.updateTargetModel();
          }
  ...
  }, [game, agent, isPaused, isTraining, speed, episode, totalReward, bestScore, epsilon, view]);
    ...
    <AnimatePresence mode="wait">
      {view === 'landing' ? (
        ...
        >
          ...
          >
            <h1 className="hero-title">NeuralSnake AI</h1>
            <p className="hero-subtitle">
              Experience the evolution of artificial intelligence. Watch a Deep Q-Network 
              learn to master the classic Snake game through trial, error, and reinforcement.
            </p>
            <button className="launch-btn" onClick={() => setView('simulation')}>
              Launch Simulation <ChevronRight size={20} className="icon-right" />
            </button>
          </motion.div>
        ...
        >
          ...
          <div className="stats-sidebar">
            ...
            <div className="stat-card">
              ...
              <input 
                id="speed-slider" type="range" min="1" max="200" 
                value={201 - speed} onChange={(e) => setSpeed(201 - parseInt(e.target.value))}
            ...
            <div className="stat-card">
              ...
              <div className="history-list">
                <button className="secondary brain-btn" onClick={() => agent.save()}><Save size={14} /> Save to Local</button>
                <button className="secondary brain-btn" onClick={async () => {
                  const success = await agent.load();
                  if (success) { setEpsilon(agent.getEpsilon()); alert('Brain Loaded!'); }
                  else { alert('No saved brain found.'); }
                }}><Upload size={14} /> Load from Local</button>
                <button className="secondary brain-btn" onClick={() => agent.export()}><Download size={14} /> Export Files</button>
              </div>
            </div>
              ...
              <div className="history-list">
                ...
                {history.slice(-5).reverse().map((h, i) => (
                  <div key={i} className="history-item">
                    ...
                    <span className={h.score > 0 ? 'score-positive' : 'score-neutral'}>Score: {h.score}</span>
                  </div>
                ))}
              </div>
            </div>
